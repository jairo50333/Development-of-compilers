...
%token BEGIN END CYCLE WHILE ASSIGN ASSIGNPLUS ASSIGNMINUS ASSIGNMULT SEMICOLON WRITE VAR PLUS MINUS MULT DIV 
        LPAREN RPAREN COLUMN MOD IF THEN ELSE REPEAT UNTIL WHILE DO 
%token <iVal> INUM 
%token <dVal> RNUM 
%token <sVal> ID

%type <eVal> expr ident T F 
%type <stVal> statement assign block cycle write empty var varlist if repeat while
